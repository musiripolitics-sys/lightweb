  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-6 px-[clamp(24px,5vw,72px)]">
        ...
        {/* Desktop nav */}
        <nav aria-label="Primary" className="hidden items-center min-[1021px]:flex">
          <ul className="flex items-center gap-[clamp(18px,2.2vw,36px)]">
              ...
              <li key={link.label}>
                <Link
                  ...
                  className={`relative text-[13px] uppercase tracking-[0.07em] no-underline transition-colors hover:text-white ${
                    active === link.label ? "text-white" : "text-white/65"
                  }`}
                >
                  {link.label}
                  {active === link.label && (
                    <span className="absolute -bottom-2 left-0 h-px w-full bg-gradient-to-r from-white/80 to-white/0" />
                  )}
                </Link>
              </li>
            ))}
            <li>
              <Link
                href="/contact"
                className="ml-2 inline-flex items-center border border-white/80 px-5 py-2.5 text-[12px] uppercase tracking-[0.1em] text-white no-underline transition-colors hover:bg-white hover:text-[#0d0d0e]"
              >
                Get a Quote
              </Link>
            </li>
        ...
        <button
          ...
          aria-expanded={open}
          onClick={() => setOpen((v) => !v)}
          className="flex flex-col gap-[5px] p-1.5 min-[1021px]:hidden"
        >
      ...
      {open && (
        <nav
          aria-label="Primary"
          className="border-t border-white/10 bg-[#0d0d0e]/95 px-[clamp(24px,5vw,72px)] py-7 backdrop-blur min-[1021px]:hidden"
        >
          <ul className="flex flex-col items-start gap-[18px]">
            ...
            <li>
              <Link
                href="/contact"
                onClick={() => setOpen(false)}
                className="mt-2 inline-flex items-center border border-white/80 px-5 py-2.5 text-[12px] uppercase tracking-[0.1em] text-white no-underline transition-colors hover:bg-white hover:text-[#0d0d0e]"
              >
                Get a Quote
              </Link>
            </li>
