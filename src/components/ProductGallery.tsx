...
export default function ProductGallery({ images, name }: { images: string[]; name: string }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="image-plate relative aspect-square overflow-hidden border border-white/10">
        <Image
          key={images[index]}
          src={images[index]}
          ...
          {images.map((src, i) => (
            <button
              ...
              className={`image-plate relative h-20 w-20 cursor-pointer overflow-hidden border transition-colors ${
                i === index ? "border-white" : "border-white/10 opacity-70 hover:opacity-100"
              }`}
            >
